...
import { salesInvoiceColumn } from '../../data/table/BillsTable'
...
const SalesOrderList = ({ add, setAdd, igstRate, sgstRate, cgstRate, utgstRate, tax, setSalesItem }) => {
    ...
    const saveHandler = (e) => {
     ...
     useEffect(() => {
        items.map((item, index) => {
            ...
            setSalesItem([...itemList, newItems])
        })
    }, [items])
    ...
    return (
        <div>
            ...
            <div className="borderImp rounded-md text-xs">
                ...
                <div className="p-1">
                    <Table columns={salesInvoiceColumn} dataSource={itemList} />
                </div>
            </div>
        </div>
    )
